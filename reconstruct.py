...
autoencoder_model.compile(loss='mean_squared_error', optimizer=Adam(lr=1e-4), metrics=['acc'])
...
for i in range(len(train_dataset_info) // batch_size):
    ...
    mini_batch_info = train_dataset_info[i*batch_size:i*batch_size+batch_size]
    ...
    for img_info in mini_batch_info:
        ...
        recons_img = recons_img[0,:,:,:]
        recons_img[:,:,1] = orig_img[:,:,1] / 255.
        imsave(recons_path+'/'+img_info['path'].split('/')[-1]+'.png', recons_img)
        n_new_img += 1
        print(str(n_new_img) + ' new images')
